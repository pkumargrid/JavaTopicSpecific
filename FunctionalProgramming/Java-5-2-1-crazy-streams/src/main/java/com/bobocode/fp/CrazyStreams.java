...
public class CrazyStreams {
    ...
    public List<Account> findAccountsByBirthdayMonth(Month birthdayMonth) {
        return accounts.stream().filter(x -> x.getMonth().getMonth().equals(birthdayMonth)).toList();
    }
    ...
    public Map<Boolean, List<Account>> partitionMaleAccounts() {
        return accounts.stream().collect(Collectors.partitioningBy(x -> x.getSex() == Sex.MALE));
    }
    ...
    public Map<String, List<Account>> groupAccountsByEmailDomain() {
        return accounts.stream().filter(account -> account.getEmail().contains("@")).collect(Collectors.
                groupingBy(account -> account.getEmail().substring(account.getEmail().indexOf("@") +1)));
    }
    ...
    public int getNumOfLettersInFirstAndLastNames() {
        return accounts.stream()
                .flatMapToInt(account -> (account.getFirstName() + account.getLastName()).chars())
                .mapToObj(ch -> (char) ch)
                .toList()
                .size();
    }
    ...
    public BigDecimal getBalanceByEmail(String email) {
        return accounts.stream().
                filter(account -> account.getEmail().equals(email)).map(Account::getBalance).
                findFirst().orElseThrow(() -> new EntityNotFoundException("Cannot find Account by email=%s".formatted(email)));
    }
    ...
    public Map<Month, String> groupCommaSeparatedFirstNamesByBirthdayMonth() {
        return accounts.stream().collect(Collectors.groupingBy(account -> account.getMonth().getMonth(),
    ...
    public Map<Character, Long> getCharacterFrequencyInFirstNames() {
        return accounts.stream()
                .flatMap(acc -> acc.getFirstName().chars().mapToObj(ch -> (char) ch))
    ...
    public Map<Character, Long> getCharacterFrequencyIgnoreCaseInFirstAndLastNames(int nameLengthBound) {
        return accounts.stream()
                ...
                .mapToObj(x -> (char)x)
                .collect(Collectors.groupingBy(x -> x, Collectors.summingLong(x -> 1)));
    }
